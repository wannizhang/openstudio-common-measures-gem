...
module OsLib_HelperMethods
  ...
  def self.get_epoch_time_from_inputs(year, date_string, time_string)
    time_object = Time.parse("#{year}-#{date_string} #{time_string} UTC")
    # Convert to Epoch time (milliseconds since 1970-01-01, required for Highcharts)
    epoch_time_milliseconds = time_object.to_i*1000
    return epoch_time_milliseconds
  end

  # all_zones_data is in the format
  #   all_zones_data = {
  #     'Zone 1' => {
  #       'Variable1' => [[timestamp, value], [timestamp, value]],
  #       'Variable2' => [[timestamp, value], [timestamp, value]]
  #     }
  #   }
  # This function calculates the min, q1, median, q3, max statistics across all zones
  # at each timestamp for all variables, used for the boxplot
  def self.compute_boxplot_data_flat_all_vars(all_zones_data)
    result = {}
    all_zones_data.each do |variable, zones_data|
      timestamps = zones_data.values.first.map { |timestamp, _| timestamp }

      # Extract variable values from all zones; each zone is a row in the matrix
      data_matrix = zones_data.values.map do |zone_data|
        zone_data.map { |_, value| value }
      end

      boxplot_data = data_matrix.transpose.map.with_index do |values_at_timestep, index|
        # Calculate the statistics for boxplot for each timestep
        sorted_values = values_at_timestep.sort
        {
          datetime: timestamps[index],
          data: [
            sorted_values.first,                         # min
            sorted_values[sorted_values.size / 4],       # q1
            sorted_values[sorted_values.size / 2],       # median
            sorted_values[(3 * sorted_values.size) / 4], # q3
            sorted_values.last                           # max
          ]
        }
      end

      result[variable] = boxplot_data
    end
    return result
  end
